...
import re
import os # Import os to safely join paths
import requests # <-- ADDED for downloading
import zipfile # <-- ADDED for unzipping

# --- Configuration ---
# Define the project folder. All paths will be relative to this.
PROJECT_FOLDER = "Text-Predicting-RNN" # <-- UPDATED to your folder name

# --- UPDATE ALL FILE PATHS ---
MODEL_PATH = os.path.join(PROJECT_FOLDER, 'best_model.keras')
TOKENIZER_PATH = os.path.join(PROJECT_FOLDER, 'tokenizer.pkl')
SEQ_LEN_PATH = os.path.join(PROJECT_FOLDER, 'sequence_len.pkl')
GLOVE_FILE = os.path.join(PROJECT_FOLDER, 'glove.6B', 'glove.6B.100d.txt') # <-- Slightly safer path
EMBEDDING_DIM = 100
# --- ADDED these two paths based on your file list ---
IMG_ACCURACY = os.path.join(PROJECT_FOLDER, 'training_accuracy.png')
IMG_LOSS = os.path.join(PROJECT_FOLDER, 'training_loss.png')
...
def load_all_artifacts():
    ...
    """
    try:
        # --- NEW FIX: Added compile=False. This stops Keras from loading the
        # --- optimizer, which is often the cause of this error.
        model = load_model(MODEL_PATH, safe_mode=False, compile=False)
    except Exception as e:
        st.error(f"Error loading model from {MODEL_PATH}: {e}")
        st.error(f"Please make sure '{MODEL_PATH}' exists.")
    ...
    try:
        ...
        st.error(f"Please make sure '{TOKENIZER_PATH}' and '{SEQ_LEN_PATH}' exist.")
        return None, None, None, None, None

    vocab_size = len(tokenizer.word_index) + 1
    
    # --- GloVe File Auto-Downloader ---
    glove_dir = os.path.join(PROJECT_FOLDER, 'glove.6B')
    glove_zip_path = os.path.join(PROJECT_FOLDER, 'glove.6B.zip')
    
    if not os.path.exists(GLOVE_FILE):
        st.warning(f"GloVe file not found at {GLOVE_FILE}. Attempting to download...")
        ...
        try:
            ...
            if not os.path.exists(glove_zip_path):
                ...
                with st.spinner(f"Downloading GloVe embeddings (~822MB)... This may take a few minutes on first load."):
            ...
            # Unzip the file
            with st.spinner(f"Unzipping GloVe embeddings..."):
                with zipfile.ZipFile(glove_zip_path, 'r') as zip_ref:
                    # Extract only the file we need into the correct directory
                    zip_ref.extract('glove.6B.100d.txt', path=glove_dir)
            
            st.success("GloVe embeddings downloaded and unzipped!")
...
# It can only be called once, and it's already in Home.py
...
if model is None:
    st.error("Model artifacts not loaded. App cannot continue.")
...
# --- Page 4: Project Overview ---
    ...
    try:
        # --- UPDATE IMAGE PATHS ---
        st.image(IMG_ACCURACY, caption='Model Training vs. Validation Accuracy')
        st.image(IMG_LOSS, caption='Model Training vs.Validation Loss')
        ...
        st.warning(f"Could not find '{IMG_ACCURACY}' or '{IMG_LOSS}'. Please run the training notebook to generate them.")
